...
static gboolean
nd_notification_box_button_release_event(GtkWidget* widget, GdkEventButton* event)
{
...
static GtkWidget* create_notification_action(NdNotificationBox* box, NdNotification* notification, const char* text, const char* key)
{
    ...
    g_signal_connect(G_OBJECT (button), "button-release-event", G_CALLBACK (on_action_clicked), box);
    return button;
}
...
static void update_notification_box(NdNotificationBox* notification_box)
{
    ...
    gtk_widget_set_size_request(notification_box->priv->summary_label, summary_width, -1);

    /* body */
    body = nd_notification_get_body(notification_box->priv->notification);
    if (validate_markup(body)) gtk_label_set_markup(GTK_LABEL(notification_box->priv->body_label), body);
    else {
        gchar* tmp;
    ...
    if (body != NULL && *body != '\0') {
        gtk_widget_set_size_request(notification_box->priv->body_label, summary_width, -1);
        have_body = TRUE;
    }
    ...
    for (i = 0; actions[i] != NULL; i += 2) {
        ...
        if (strcasecmp(actions[i], "default") != 0) {
            ...
            button = create_notification_action(notification_box, notification_box->priv->notification, l, actions[i]);
...
static void nd_notification_box_init(NdNotificationBox* notification_box)
{
    ...
    gtk_widget_set_size_request(notification_box->priv->icon, BODY_X_OFFSET, -1);
    ...
    gtk_widget_set_valign(notification_box->priv->close_button, GTK_ALIGN_START);
    gtk_widget_show(notification_box->priv->close_button);

    gtk_box_pack_start(GTK_BOX(box), notification_box->priv->close_button, FALSE, FALSE, 0);
