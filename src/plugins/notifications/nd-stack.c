...
static void get_origin_coordinates(NdStackLocation stack_location, GdkRectangle* workarea, gint* x, gint* y, gint* shiftx, gint* shifty, gint width, gint height)
{
    ...
    case ND_STACK_LOCATION_UNKNOWN: default: g_assert_not_reached();
    }
}

static void translate_coordinates(NdStackLocation stack_location, GdkRectangle* workarea, gint* x, gint* y, gint* shiftx, gint* shifty, gint width, gint height)
{
    switch (stack_location) {
    case ND_STACK_LOCATION_TOP_LEFT:
        *x = workarea->x;
...
static void location_changed_cb(GSettings* settings, const char* key, NdStack* stack)
{
...
static void nd_stack_init(NdStack* stack)
{
    ...
    g_signal_connect(stack->priv->settings, "changed::location", G_CALLBACK (location_changed_cb), stack);
}
...
static void add_padding_to_rect(GdkRectangle* rect)
{
...
static void nd_stack_shift_notifications(NdStack* stack, GDBubble* bubble, GList** nw_l, gint init_width, gint init_height, gint* nw_x, gint* nw_y)
{
    ...
    get_origin_coordinates(stack->priv->location, &workarea, &x, &y, &shiftx, &shifty, init_width, init_height);
    ...
    for (i = 0, l = stack->priv->bubbles; l != NULL; i++, l = l->next) {
        GDBubble* nw2 = GD_BUBBLE(l->data);
        GtkRequisition req;

        if (bubble == NULL || nw2 != bubble) {
            gtk_widget_get_preferred_size(GTK_WIDGET(nw2), NULL, &req);

            translate_coordinates(stack->priv->location, &workarea, &x, &y, &shiftx, &shifty, req.width, req.height + NOTIFY_STACK_SPACING);
            positions[i].x = x;
            positions[i].y = y;
        }
        else if (nw_l != NULL) {
...
static void update_position(NdStack* stack)
{
    nd_stack_shift_notifications(stack, NULL, /* window */
                                 NULL, /* list pointer */
                                 0, /* init width */
                                 0, /* init height */
                                 NULL, /* out window x */
...
static gboolean update_position_idle(NdStack* stack)
{
...
void nd_stack_add_bubble(NdStack* stack, GDBubble* bubble)
{
    ...
    nd_stack_shift_notifications(stack, bubble, NULL, req.width, req.height + NOTIFY_STACK_SPACING, &x, &y);
    gtk_widget_show(GTK_WIDGET(bubble));
    gtk_window_move(GTK_WINDOW(bubble), x, y);

    g_signal_connect_object(bubble, "destroy", G_CALLBACK(nd_stack_remove_bubble), stack, G_CONNECT_SWAPPED);
...
void nd_stack_remove_bubble(NdStack* stack, GDBubble* bubble)
{
    ...
    nd_stack_shift_notifications(stack, bubble, &remove_l, 0, 0, NULL, NULL);

    if (remove_l != NULL) stack->priv->bubbles = g_list_delete_link(stack->priv->bubbles, remove_l);

    if (gtk_widget_get_realized(GTK_WIDGET(bubble))) gtk_widget_unrealize(GTK_WIDGET(bubble));
}

void nd_stack_remove_all(NdStack* stack)
{
    GList* bubbles = g_list_copy(stack->priv->bubbles);
