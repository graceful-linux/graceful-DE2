...
create_dock(NdQueue* queue)
{
    ...
    gtk_widget_add_events(queue->priv->dock, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);

    gtk_widget_set_name(queue->priv->dock, "notification-popup-window");
    g_signal_connect(queue->priv->dock, "grab-notify", G_CALLBACK (on_dock_grab_notify), queue);
    g_signal_connect(queue->priv->dock, "grab-broken-event", G_CALLBACK (on_dock_grab_broken_event), queue);
    g_signal_connect(queue->priv->dock, "key-release-event", G_CALLBACK (on_dock_key_release), queue);
    g_signal_connect(queue->priv->dock, "button-press-event", G_CALLBACK (on_dock_button_press), queue);
#if 0
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(queue->priv->dock_scrolled_window), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
    gtk_widget_set_size_request(queue->priv->dock_scrolled_window, WIDTH, -1);
...
maybe_show_notification(NdQueue* queue)
{
    ...
    NdNotification* notification = g_hash_table_lookup(queue->priv->notifications, id);
    g_assert(notification != NULL);

    GDBubble* bubble = gd_bubble_new_for_notification(notification);
...
update_dock(NdQueue* queue)
{
    ...
    if (child != NULL) gtk_container_remove(GTK_CONTAINER(queue->priv->dock_scrolled_window), child);

    child = gtk_box_new(GTK_ORIENTATION_VERTICAL, 6);
    gtk_container_add(GTK_CONTAINER(queue->priv->dock_scrolled_window), child);

    gtk_container_set_focus_hadjustment(GTK_CONTAINER(child), gtk_scrolled_window_get_hadjustment(GTK_SCROLLED_WINDOW(queue->priv->dock_scrolled_window)));
    gtk_container_set_focus_vadjustment(GTK_CONTAINER(child), gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(queue->priv->dock_scrolled_window)));
    ...
    if (status_icon != NULL) {
        G_GNUC_BEGIN_IGNORE_DEPRECATIONS visible = gtk_status_icon_get_visible(status_icon);
    ...
    if (visible) {
        ...
        G_GNUC_BEGIN_IGNORE_DEPRECATIONS gtk_status_icon_get_geometry(status_icon, &screen, &area, NULL);
        G_GNUC_END_IGNORE_DEPRECATIONS monitor = gdk_display_get_monitor_at_point(gdk_screen_get_display(screen), area.x, area.y);
        gdk_monitor_get_geometry(monitor, &area);
        height = MIN(height, (area.height / 2));
        gtk_widget_set_size_request(queue->priv->dock_scrolled_window, WIDTH, height);
    }
...
update_idle(NdQueue* queue)
{
    ...
    if (num > 0) {
        ...
        if (queue->priv->status_icon == NULL) {
            G_GNUC_BEGIN_IGNORE_DEPRECATIONS queue->priv->status_icon = gtk_status_icon_new();
            gtk_status_icon_set_title(queue->priv->status_icon, _("Notifications"));
            G_GNUC_END_IGNORE_DEPRECATIONS g_signal_connect(queue->priv->status_icon, "activate", G_CALLBACK (on_status_icon_activate), queue);
            g_signal_connect(queue->priv->status_icon, "popup-menu", G_CALLBACK (on_status_icon_popup_menu), queue);
            g_signal_connect(queue->priv->status_icon, "notify::visible", G_CALLBACK (on_status_icon_visible_notify), queue);
        }
        ...
        G_GNUC_BEGIN_IGNORE_DEPRECATIONS gtk_numerable_icon_set_count(GTK_NUMERABLE_ICON(queue->priv->numerable_icon), num);
        gtk_status_icon_set_from_gicon(queue->priv->status_icon, queue->priv->numerable_icon);
        gtk_status_icon_set_visible(queue->priv->status_icon, TRUE);
        G_GNUC_END_IGNORE_DEPRECATIONS maybe_show_notification(queue);
    }
    else {
        if (gtk_widget_get_visible(queue->priv->dock)) {
            popdown_dock(queue);
