...
enum { PROP_0, PROP_SCHEMA_ID, LAST_PROP };
...
G_DEFINE_TYPE(GDBG, gd_bg, G_TYPE_OBJECT)

static GdkPixbuf* pixbuf_scale_to_fit(GdkPixbuf* src, int max_width, int max_height);
static GdkPixbuf* pixbuf_scale_to_min(GdkPixbuf* src, int min_width, int min_height);
static void pixbuf_draw_gradient(GdkPixbuf* pixbuf, gboolean horizontal, GdkRGBA* c1, GdkRGBA* c2, GdkRectangle* rect);
static void pixbuf_tile(GdkPixbuf* src, GdkPixbuf* dest);
static void pixbuf_blend(GdkPixbuf* src, GdkPixbuf* dest, int src_x, int src_y, int width, int height, int dest_x, int dest_y, double alpha);

static GdkPixbuf* get_pixbuf_for_size(GDBG* bg, gint num_monitor, int width, int height);
static void clear_cache(GDBG* bg);
static time_t get_mtime(const char* filename);

static void
color_from_string(const char* string, GdkRGBA* colorp)
{
...
{
    return g_strdup_printf("#%02x%02x%02x", (int)(0.5 + color->red * 255), (int)(0.5 + color->green * 255), (int)(0.5 + color->blue * 255));
}
...
static void
queue_changed(GDBG* bg)
{
    if (bg->changed_id > 0) {
        g_source_remove(bg->changed_id);
    }

    bg->changed_id = g_timeout_add_full(G_PRIORITY_LOW, 100, (GSourceFunc)do_changed, bg, NULL);
}

static gboolean
do_transitioned(GDBG* bg)
{
...
static void
queue_transitioned(GDBG* bg)
{
    if (bg->transitioned_id > 0) {
        g_source_remove(bg->transitioned_id);
    }

    bg->transitioned_id = g_timeout_add_full(G_PRIORITY_LOW, 100, (GSourceFunc)do_transitioned, bg, NULL);
}

static gboolean
bg_gsettings_mapping(GVariant* value, gpointer* result, gpointer user_data)
{
...
static inline gchar*
get_wallpaper_cache_prefix_name(gint num_monitor, GDesktopBackgroundStyle placement, gint width, gint height)
{
    return g_strdup_printf("%i_%i_%i_%i", num_monitor, (gint)placement, width, height);
}

static char*
get_wallpaper_cache_filename(const char* filename, gint num_monitor, GDesktopBackgroundStyle placement, gint width, gint height)
{
...
static void
cleanup_cache_for_monitor(gchar* cache_dir, gint num_monitor)
{
    GDir* g_cache_dir;
...
static gboolean
cache_file_is_valid(const char* filename, const char* cache_filename)
{
    time_t mtime;
    time_t cache_mtime;
...
static void
refresh_cache_file(GDBG* bg, GdkPixbuf* new_pixbuf, gint num_monitor, gint width, gint height)
{
    gchar* cache_filename;
    gchar* cache_dir;
    ...
    if (!cache_file_is_valid(bg->filename, cache_filename)) {
        ...
        if (format != NULL) {
            ...
            if (strcmp(format_name, "jpeg") == 0) gdk_pixbuf_save(new_pixbuf, cache_filename, format_name, NULL, "quality", "100", NULL);
            else gdk_pixbuf_save(new_pixbuf, cache_filename, format_name, NULL, NULL);
...
static void
file_changed(GFileMonitor* file_monitor, GFile* child, GFile* other_file, GFileMonitorEvent event_type, gpointer user_data)
{
    GDBG* bg = GD_BG(user_data);
...
static void
draw_color_area(GDBG* bg, GdkPixbuf* dest, GdkRectangle* rect)
{
    guint32 pixel;
    GdkRectangle extent;
    ...
    if (!gdk_rectangle_intersect(rect, &extent, rect)) return;

    switch (bg->color_type) {
    case G_DESKTOP_BACKGROUND_SHADING_SOLID:
        /* not really a big deal to ignore the area of interest */
        pixel = ((int)(0.5 + bg->primary.red * 255) << 24) | ((int)(0.5 + bg->primary.green * 255) << 16) | ((int)(0.5 + bg->primary.blue * 255) << 8) | (0xff);
...
static void
draw_color(GDBG* bg, GdkPixbuf* dest)
{
    GdkRectangle rect;
    rect.x = 0;
    rect.y = 0;
...
static void
draw_color_each_monitor(GDBG* bg, GdkPixbuf* dest, GdkDisplay* display, gint scale)
{
    int n_monitors;
    int i;
...
pixbuf_clip_to_fit(GdkPixbuf* src, int max_width, int max_height)
{
    ...
    if (src_width < max_width && src_height < max_height) return g_object_ref(src);

    w = MIN(src_width, max_width);
    h = MIN(src_height, max_height);

    pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, gdk_pixbuf_get_has_alpha(src), 8, w, h);
...
static GdkPixbuf*
get_scaled_pixbuf(GDesktopBackgroundStyle placement, GdkPixbuf* pixbuf, int width, int height, int* x, int* y, int* w, int* h)
{
    GdkPixbuf* new;
    ...
    case G_DESKTOP_BACKGROUND_STYLE_WALLPAPER: default:
...
static void
draw_image_area(GDBG* bg, gint num_monitor, GdkPixbuf* pixbuf, GdkPixbuf* dest, GdkRectangle* area)
{
    ...
    case G_DESKTOP_BACKGROUND_STYLE_SPANNED:
        ...
        break;
    case G_DESKTOP_BACKGROUND_STYLE_NONE: default: g_assert_not_reached();
        break;
    }
...
static void
draw_once(GDBG* bg, GdkPixbuf* dest)
{
    GdkRectangle rect;
    GdkPixbuf* pixbuf;
    gint num_monitor;
    ...
    if (pixbuf) {
        ...
        draw_image_area(bg, num_monitor, pixbuf, dest, &rect);
        g_object_unref(pixbuf);
    }
}

static void
draw_each_monitor(GDBG* bg, GdkPixbuf* dest, GdkDisplay* display, gint scale)
{
    int n_monitors;
    int i;
...
static void
gd_bg_draw_at_scale(GDBG* bg, GdkPixbuf* dest, gint scale, GdkDisplay* display, gboolean is_root)
{
    if (is_root && (bg->placement != G_DESKTOP_BACKGROUND_STYLE_SPANNED)) {
        draw_color_each_monitor(bg, dest, display, scale);

        if (bg->placement != G_DESKTOP_BACKGROUND_STYLE_NONE) draw_each_monitor(bg, dest, display, scale);
    }
    else {
        draw_color(bg, dest);

        if (bg->placement != G_DESKTOP_BACKGROUND_STYLE_NONE) draw_once(bg, dest);
    }
}

static void
gd_bg_get_pixmap_size(GDBG* bg, int width, int height, int* pixmap_width, int* pixmap_height)
{
    int dummy;
    ...
    *pixmap_height = height;
    ...
    if (!bg->filename) {
        ...
        case G_DESKTOP_BACKGROUND_SHADING_HORIZONTAL:
        case G_DESKTOP_BACKGROUND_SHADING_VERTICAL: default:
            break;
        }
...
static GdkPixbuf*
blend(GdkPixbuf* p1, GdkPixbuf* p2, double alpha)
{
    GdkPixbuf* result = gdk_pixbuf_copy(p1);
    GdkPixbuf* tmp;

    if (gdk_pixbuf_get_width(p2) != gdk_pixbuf_get_width(p1) || gdk_pixbuf_get_height(p2) != gdk_pixbuf_get_height(p1)) {
        tmp = gdk_pixbuf_scale_simple(p2, gdk_pixbuf_get_width(p1), gdk_pixbuf_get_height(p1), GDK_INTERP_BILINEAR);
    }
    else {
        tmp = g_object_ref(p2);
    }
...
typedef enum { PIXBUF, SLIDESHOW } FileType;
...
file_cache_lookup(GDBG* bg, FileType type, const char* filename)
{
    GList* list;
...
file_cache_entry_new(GDBG* bg, FileType type, const char* filename)
{
...
static void
file_cache_add_pixbuf(GDBG* bg, const char* filename, GdkPixbuf* pixbuf)
{
...
static void
file_cache_add_slide_show(GDBG* bg, const char* filename, GnomeBGSlideShow* show)
{
...
static GdkPixbuf*
load_from_cache_file(GDBG* bg, const char* filename, gint num_monitor, gint best_width, gint best_height)
{
    GdkPixbuf* pixbuf = NULL;
    gchar* cache_filename;

    cache_filename = get_wallpaper_cache_filename(filename, num_monitor, bg->placement, best_width, best_height);
    if (cache_file_is_valid(filename, cache_filename)) pixbuf = gdk_pixbuf_new_from_file(cache_filename, NULL);
    g_free(cache_filename);
...
static GdkPixbuf*
get_as_pixbuf_for_size(GDBG* bg, const char* filename, gint num_monitor, gint best_width, gint best_height)
{
    ...
    if ((ent = file_cache_lookup(bg, PIXBUF, filename))) {
        ...
        if (num_monitor != -1) pixbuf = load_from_cache_file(bg, filename, num_monitor, best_width, best_height);
        ...
        if (!pixbuf) {
            ...
            if (tmp != NULL && strcmp(tmp, "svg") == 0 && (best_width > 0 && best_height > 0) && (bg->placement == G_DESKTOP_BACKGROUND_STYLE_STRETCHED || bg->placement == G_DESKTOP_BACKGROUND_STYLE_SCALED || bg->placement ==
                G_DESKTOP_BACKGROUND_STYLE_ZOOM)) pixbuf = gdk_pixbuf_new_from_file_at_size(filename, best_width, best_height, NULL);
            else pixbuf = gdk_pixbuf_new_from_file(filename, NULL);
            g_free(tmp);
        }

        if (pixbuf) file_cache_add_pixbuf(bg, filename, pixbuf);
...
read_slideshow_file(const char* filename, GError** err)
{
...
static GnomeBGSlideShow*
get_as_slideshow(GDBG* bg, const char* filename)
{
    const FileCacheEntry* ent;
...
static void
blow_expensive_caches_in_idle(GDBG* bg)
{
...
static double
get_slide_timeout(gboolean is_fixed, gdouble duration)
{
    double timeout;
    if (is_fixed) {
        timeout = duration;
...
static void
ensure_timeout(GDBG* bg, gdouble timeout)
{
    if (!bg->timeout_id) {
        /* G_MAXUINT means "only one slide" */
        if (timeout < G_MAXUINT) {
            bg->timeout_id = g_timeout_add_full(G_PRIORITY_LOW, timeout * 1000, on_timeout, bg, NULL);
        }
    }
}
...
static GdkPixbuf*
get_pixbuf_for_size(GDBG* bg, gint num_monitor, gint best_width, gint best_height)
{
    ...
    if (!hit_cache && bg->filename) {
        ...
        if (!bg->pixbuf_cache) {
            ...
            if (show) {
                ...
                gnome_bg_slide_show_get_current_slide(show, best_width, best_height, &alpha, &duration, &is_fixed, &file1, &file2);
        ...
        /* If the next slideshow step is a long time away then
           ...
           the cache */
        if (time_until_next_change > KEEP_EXPENSIVE_CACHE_SECS) blow_expensive_caches_in_idle(bg);
    }
...
static gboolean
is_different(GDBG* bg, const char* filename)
{
    if (!filename && bg->filename) {
        ...
        if (mtime != bg->file_mtime) return TRUE;
...
static void
pixbuf_average_value(GdkPixbuf* pixbuf, GdkRGBA* result)
{
...
pixbuf_scale_to_min(GdkPixbuf* src, int min_width, int min_height)
{
    ...
    dest = gdk_pixbuf_new(GDK_COLORSPACE_RGB, gdk_pixbuf_get_has_alpha(src), 8, min_width, min_height);
    if (!dest) return NULL;

    /* crop the result */
    gdk_pixbuf_scale(src, dest, 0, 0, min_width, min_height, (new_width - min_width) / -2, (new_height - min_height) / -2, factor, factor, GDK_INTERP_BILINEAR);
    return dest;
}

static guchar*
create_gradient(const GdkRGBA* primary, const GdkRGBA* secondary, int n_pixels)
{
...
static void
pixbuf_draw_gradient(GdkPixbuf* pixbuf, gboolean horizontal, GdkRGBA* primary, GdkRGBA* secondary, GdkRectangle* rect)
{
    int width;
    int height;
    int rowstride;
...
static void
pixbuf_blend(GdkPixbuf* src, GdkPixbuf* dest, int src_x, int src_y, int src_width, int src_height, int dest_x, int dest_y, double alpha)
{
    ...
    gdk_pixbuf_composite(src, dest, dest_x, dest_y, src_width, src_height, offset_x, offset_y, 1, 1, GDK_INTERP_NEAREST, alpha * 0xFF + 0.5);
}
...
static Pixmap
create_persistent_pixmap(int width, int height)
{
    Display* xdisplay;
    int depth;
    Pixmap pixmap;
    ...
    pixmap = XCreatePixmap(xdisplay, XDefaultRootWindow(xdisplay), width, height, depth);
...
create_persistent_surface(GdkDisplay* display, int width, int height)
{
    Display* xdisplay;
    Pixmap pixmap;
    Visual* xvisual;
    ...
    surface = cairo_xlib_surface_create(xdisplay, pixmap, xvisual, width, height);

    return surface;
}

static gboolean
is_valid_pixmap(GdkDisplay* display, Pixmap pixmap)
{
    Display* xdisplay;
    Window root_return;
    int x_return;
    ...
    status = XGetGeometry(xdisplay, pixmap, &root_return, &x_return, &y_return, &width_return, &height_return, &border_width_return, &depth_return);

    if (gdk_x11_display_error_trap_pop(display) != 0 || status == 0) return FALSE;
...
get_root_pixmap(GdkDisplay* display)
{
    ...
    result = XGetWindowProperty(xdisplay, XDefaultRootWindow(xdisplay), xrootpmap_id_atom, 0l, 1l, False, XA_PIXMAP, &actual_type, &actual_format, &n_items, &bytes_after, &prop);

    if (result != Success || actual_type != XA_PIXMAP || actual_format != 32 || n_items != 1) {
        XFree(prop);
    ...
    if (!is_valid_pixmap(display, pixmap)) return None;

    return pixmap;
}

static void
set_root_pixmap(GdkDisplay* display, Pixmap pixmap)
{
    Display* xdisplay;
    ...
    result = XGetWindowProperty(xdisplay, XDefaultRootWindow(xdisplay), esetroot_pmap_id_atom, 0l, 1l, False, XA_PIXMAP, &actual_type, &actual_format, &n_items, &bytes_after, &prop);

    if (result == Success && actual_type == XA_PIXMAP && actual_format == 32 && n_items == 1 && prop != NULL) {
        gdk_x11_display_error_trap_push(display);
    ...
    XChangeProperty(xdisplay, XDefaultRootWindow(xdisplay), esetroot_pmap_id_atom, XA_PIXMAP, 32, PropModeReplace, (guchar*)&pixmap, 1);

    XChangeProperty(xdisplay, XDefaultRootWindow(xdisplay), xrootpmap_id_atom, XA_PIXMAP, 32, PropModeReplace, (guchar*)&pixmap, 1);
}

static void
set_average_color(GdkDisplay* display, GdkRGBA* color)
{
    Display* xdisplay;
    ...
    representative_colors_atom = XInternAtom(xdisplay, "_GNOME_BACKGROUND_REPRESENTATIVE_COLORS", False);
    ...
    if (color != NULL) {
        ...
        XChangeProperty(xdisplay, XDefaultRootWindow(xdisplay), representative_colors_atom, XA_STRING, 8, PropModeReplace, (guchar*)string, strlen(string) + 1);

        g_free(string);
    }
    else {
        XDeleteProperty(xdisplay, XDefaultRootWindow(xdisplay), representative_colors_atom);
    }
}

static gboolean
settings_change_event_cb(GSettings* settings, void* keys, int n_keys, GDBG* self)
{
    gd_bg_load_from_preferences(self);

    return TRUE;
}

static gboolean
color_scheme_changed_cb(GSettings* interface_settings, const char* key, GDBG* self)
{
    gd_bg_load_from_preferences(self);
...
static void
gd_bg_constructed(GObject* object)
{
    GDBG* self;
    ...
    G_OBJECT_CLASS(gd_bg_parent_class)->constructed(object);
    ...
    self->change_event_id = g_signal_connect(self->settings, "change-event", G_CALLBACK (settings_change_event_cb), self);

    self->changed_color_scheme_id = g_signal_connect(self->interface_settings, "changed::color-scheme", G_CALLBACK (color_scheme_changed_cb), self);
}

static void
gd_bg_dispose(GObject* object)
{
    GDBG* self;
    ...
    G_OBJECT_CLASS(gd_bg_parent_class)->dispose(object);
}

static void
gd_bg_finalize(GObject* object)
{
    GDBG* self;
    ...
    G_OBJECT_CLASS(gd_bg_parent_class)->finalize(object);
}

static void
gd_bg_set_property(GObject* object, unsigned int property_id, const GValue* value, GParamSpec* pspec)
{
    GDBG* self;
    ...
    switch (property_id) {
    case PROP_SCHEMA_ID: g_assert(self->schema_id == NULL);
...
{
    bg_properties[PROP_SCHEMA_ID] = g_param_spec_string("schema-id", "schema-id", "schema-id", NULL, G_PARAM_CONSTRUCT_ONLY | G_PARAM_WRITABLE | G_PARAM_STATIC_STRINGS);

    g_object_class_install_properties(object_class, LAST_PROP, bg_properties);
}

static void
gd_bg_class_init(GDBGClass* self_class)
{
    GObjectClass* object_class;

    object_class = G_OBJECT_CLASS(self_class);

    object_class->constructed = gd_bg_constructed;
    object_class->dispose = gd_bg_dispose;
    object_class->finalize = gd_bg_finalize;
    object_class->set_property = gd_bg_set_property;

    install_properties(object_class);

    signals[CHANGED] = g_signal_new("changed", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    signals[TRANSITIONED] = g_signal_new("transitioned", G_OBJECT_CLASS_TYPE(object_class), G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void
gd_bg_init(GDBG* self)
{
}

GDBG*
gd_bg_new(const char* schema_id)
{
    return g_object_new(GD_TYPE_BG, "schema-id", schema_id, NULL);
}

void
gd_bg_load_from_preferences(GDBG* self)
{
    char* tmp;
    char* filename;
    ...
    g_return_if_fail(GD_IS_BG (self));

    /* Filename */
    tmp = g_settings_get_string(self->interface_settings, IFACE_KEY_COLOR_SCHEME);
    if (self->has_dark_key && strcmp(tmp, "prefer-dark") == 0)
        filename = g_settings_get_mapped(self->settings, BG_KEY_PICTURE_URI_DARK, bg_gsettings_mapping, NULL);
    else
        filename = g_settings_get_mapped(self->settings, BG_KEY_PICTURE_URI, bg_gsettings_mapping, NULL);
    g_free(tmp);
    ...
    gd_bg_set_rgba(self, ctype, &c1, &c2);
    gd_bg_set_placement(self, placement);
    gd_bg_set_filename(self, filename);

    g_free(filename);
}

void
gd_bg_save_to_preferences(GDBG* self)
{
    gchar* primary;
    gchar* secondary;
    gchar* uri;
    ...
    if (self->filename != NULL) uri = g_filename_to_uri(self->filename, NULL, NULL);

    if (uri == NULL) uri = g_strdup("");
...
void
gd_bg_set_filename(GDBG* self, const char* filename)
{
    g_return_if_fail(self != NULL);

    if (!is_different(self, filename)) return;
    ...
    if (self->filename != NULL) {
        ...
        g_signal_connect(self->file_monitor, "changed", G_CALLBACK (file_changed), self);
    }
...
void
gd_bg_set_placement(GDBG* self, GDesktopBackgroundStyle placement)
{
    g_return_if_fail(self != NULL);
...
void
gd_bg_set_rgba(GDBG* self, GDesktopBackgroundShading type, GdkRGBA* primary, GdkRGBA* secondary)
{
    g_return_if_fail(self != NULL);
    g_return_if_fail(primary != NULL);

    if (self->color_type != type || !gdk_rgba_equal(&self->primary, primary) || (secondary != NULL && !gdk_rgba_equal(&self->secondary, secondary))) {
        self->color_type = type;
        self->primary = *primary;

        if (secondary != NULL) self->secondary = *secondary;
...
cairo_surface_t*
gd_bg_create_surface(GDBG* self, GdkWindow* window, int width, int height, bool root)
{
    gint scale;
    int pm_width;
    int pm_height;
    ...
    if (self->pixbuf_cache && gdk_pixbuf_get_width(self->pixbuf_cache) != width && gdk_pixbuf_get_height(self->pixbuf_cache) != height) {
    ...
    gd_bg_get_pixmap_size(self, width, height, &pm_width, &pm_height);

    if (root) {
        surface = create_persistent_surface(gdk_window_get_display(window), scale * pm_width, scale * pm_height);

        cairo_surface_set_device_scale(surface, scale, scale);
    }
    else {
        surface = gdk_window_create_similar_surface(window, CAIRO_CONTENT_COLOR, pm_width, pm_height);
    }
    ...
    if (self->filename == NULL && self->color_type == G_DESKTOP_BACKGROUND_SHADING_SOLID) {
        ...
        pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, FALSE, 8, scale * width, scale * height);

        gd_bg_draw_at_scale(self, pixbuf, scale, gdk_window_get_display(window), root);
    ...
    cairo_surface_set_user_data(surface, &average_color_key, gdk_rgba_copy(&average), (cairo_destroy_func_t)gdk_rgba_free);

    return surface;
}

void
gd_bg_set_surface_as_root(GdkDisplay* display, cairo_surface_t* surface)
{
    Display* xdisplay;
    Pixmap pixmap;
    GdkRGBA* average;
...
cairo_surface_t*
gd_bg_get_surface_from_root(GdkDisplay* display, int width, int height)
{
    Display* xdisplay;
    Pixmap root_pixmap;
    GdkScreen* screen;
    ...
    if (root_pixmap != None) {
        ...
        pixmap_surface = cairo_xlib_surface_create(xdisplay, root_pixmap, xvisual, width * scale, height * scale);
    }

    if (pixmap_surface != NULL) {
        cairo_t* cr;

        surface = cairo_surface_create_similar(pixmap_surface, CAIRO_CONTENT_COLOR, width * scale, height * scale);
    ...
    return gdk_window_create_similar_surface(root, CAIRO_CONTENT_COLOR, width, height);
}

GdkRGBA*
gd_bg_get_average_color_from_surface(cairo_surface_t* surface)
{
